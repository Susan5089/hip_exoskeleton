...
class SimulationNN(nn.Module):
    ...
    def _BuildActionFilter(self):
        ...
        a_filter = action_filter.ActionFilterButter(sampling_rate=sampling_rate, num_joints=num_joints, 
                                                    filter_low_cut = 0, filter_high_cut = 5)
        return a_filter
    ...
    def forward(self, x):
        ...
        # p_out = MultiVariateNormal(p_out, torch.zeros(self.num_actions))
        
        if np.any(np.isnan(self.v_fc1.weight.detach().numpy())):
            print("here")
    ...
    def get_action(self, s):
        ts = torch.tensor(s)
        p, _ = self.forward(ts)
        #print(p.loc.cpu().detach().numpy().flatten())
...
class SimulationHumanNN(nn.Module):
    def __init__(self, num_states, num_actions):
        ...
        self._action_filter_human = self._BuildHumanActionFilter()

    def _BuildHumanActionFilter(self):
        sampling_rate = 30
        num_joints = self.num_actions
        a_filter = action_filter.ActionFilterButter(sampling_rate=sampling_rate, num_joints=num_joints, 
                                                    filter_low_cut = 0, filter_high_cut = 7)
        return a_filter

    def _FilterHumanAction(self, action):
        if sum(self._action_filter_human.xhist[0])[0] == 0:
            self._action_filter_human.init_history(action)

        return self._action_filter_human.filter(action)
    ...
    def get_humanaction(self, s):
        ts = torch.tensor(s)
        p, _ = self.forward(ts)
        p_ = self._FilterHumanAction(p.loc.cpu().detach().numpy())
