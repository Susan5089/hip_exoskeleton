...
Window(std::vector<MASS::Environment*> mEnvs)
	:mEnvs(mEnvs), mFocus(true),mSimulating(false),mDrawOBJ(false),mDrawCollision(true),mDrawContact(true), mDrawground(true), mDrawShape(true), mDrawMuscles(true),
...
Window::
Window(std::vector<MASS::Environment*> mEnvs, const std::string& nn_path,const std::string& human_muscle_nn_path)
	:Window(mEnvs, nn_path)
{
...
{	
	...
	// }
	// DrawSkeleton(mEnvs[0]->GetCharacter()->GetSkeleton());
	Eigen::VectorXd mTargetPositions = mEnvs[0]->GetCharacter()->GetSkeleton()->getPositions();

	int offset_x[5] = {-4, -2, 0, 2, 4};
	int offset_y[5] = {-4, -2, 0, 2, 4};

	for(int _i=0; _i<5; _i++){
		for(int _j=0; _j<5; _j++){
			mTargetPositions[3] += offset_x[_i];
			mTargetPositions[5] += offset_y[_j];
			mEnvs[0]->GetCharacter()->GetSkeleton()->setPositions(mTargetPositions);
			DrawSkeleton(mEnvs[0]->GetCharacter()->GetSkeleton());
			DrawMuscles(mEnvs[0]->GetCharacter()->GetMuscles());
			mTargetPositions[3] -= offset_x[_i];
			mTargetPositions[5] -= offset_y[_j];
			mEnvs[0]->GetCharacter()->GetSkeleton()->setPositions(mTargetPositions);
		}
...
Step()
{	
	if (!mEnvs[0]->GetUsetargetvisual())
	{ 
		MASS::Environment* mEnv;
		Eigen::VectorXd action;
		Eigen::VectorXd action_human;
		int num;
		Eigen::VectorXd action_full;
		plt::ion();

		for(int env_idx = 0; env_idx != mEnvs.size(); env_idx++) {
			mEnv = mEnvs[env_idx];
			num = mEnv->GetSimulationHz()/mEnv->GetControlHz();
			if(mNNLoaded)
				action = GetActionFromNN(mEnv);
			else
				action = Eigen::VectorXd::Zero(mEnv->GetNumAction());
			if(mHumanNNLoaded)
				action_human = GetActionFromHumanNN(mEnv);
			else
				action_human = Eigen::VectorXd::Zero(mEnv->GetNumHumanAction());

			action_full = Eigen::VectorXd::Zero(action.rows()); 
			if(mEnv->GetUseSymmetry())
			{
			...
			if(mEnv->GetUseMuscleNN())
			{
				...
				for(int i=0;i<num;i+=inference_per_sim){
					...
					mEnv->SetActivationLevels(GetActivationFromNN(mt, mEnv));
...
{
	for(int env_idx = 0; env_idx != mEnvs.size(); env_idx++) {
		MASS::Environment* mEnv = mEnvs[env_idx];
		mEnv->Reset();
	}
}
void
...
Eigen::VectorXd
Window::
GetActionFromNN(MASS::Environment* mEnv)
{
	p::object get_action;
...
Eigen::VectorXd
Window::
GetActionFromHumanNN(MASS::Environment* mEnv)
{
...
Eigen::VectorXd
Window::
GetActivationFromNN(const Eigen::VectorXd& mt, Environment* mEnv)
{
