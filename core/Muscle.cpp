...
AddAnchor(const dart::dynamics::SkeletonPtr& skel,dart::dynamics::BodyNode* bn,const Eigen::Vector3d& glob_pos,int num_related_bodies)
{
	...
	if(distance[index_sort_by_distance[0]]<0.08)
	{
		...
		// for Pelvis bodyNode, we don't consider its parentBodyNode as its parent is set to exo_Pelvis. 
		if(lbs_body_nodes[0]->getParentBodyNode()!=nullptr && lbs_body_nodes[0]->getName() != "Pelvis") 
		{
...
GetForce()
{	
	//std::cout << "---" <<Getf_A() << "  " <<  Getf_p()<< std::endl;
...
GetJacobianTranspose()
{
	...
	// int ch = this->Gethumandof();
	// int ch1 =  this->Gethumanbodynodes();
