...
public:
	...
	void LoadExoInitialState(const std::string& path);
	...
	void SetMusclelimitforVisual (double muscle_max_visual, double muscle_min_visual) {mMuscle_min_visual = muscle_min_visual;mMuscle_max_visual= muscle_max_visual;};
	...
	Eigen::VectorXd GetExoInitialState(){return mExoInitialState;}
	...
	const std::map<std::string,double>& GetMusclelimitforVisualmap(){return mMusclelimitforVisual;}
	const std::map<std::string,std::vector<std::string>>& GetRelatedMuscleMap() {return mrelatedmusclemap;}
	...
	int Gethumanbodynodes() {return mhumanbodynodes;}
	void ChangeHumanframe();
	Eigen::Isometry3d GetmTc() {return mTc;}
	void SetmTc(Eigen::Isometry3d a) {mTc = a;}
	double GetMuscleminlimitforVisual(){return mMuscle_min_visual;}
	double GetMusclemaxlimitforVisual(){return mMuscle_max_visual;}
	int GetActiveMuscleNum(){return num_active_muscle;}
	void SetMaxIsometricMuscleForce(double f_max); 
	void MergeHumanandExo();

public:
	Eigen::Isometry3d humanInitialRelativeTransform; 
	Environment* mEnv;
	dart::dynamics::SkeletonPtr mSkeleton, mExo;
	BVH* mBVH;
	...
	Eigen::VectorXd mExoInitialState;
	...
	std::map<std::string,double> mMusclelimitforVisual;
	std::map<std::string,std::vector<std::string>> mrelatedmusclemap;
	double mMuscle_max_visual, mMuscle_min_visual;
};
};
