...
MASS::
MakePrismaticJointProperties(const std::string& name,const Eigen::Vector3d& axis,const Eigen::Isometry3d& parent_to_joint,const Eigen::Isometry3d& child_to_joint,const Eigen::Vector1d& lower,const Eigen::Vector1d& upper)
{
	PrismaticJoint::Properties* props = new PrismaticJoint::Properties();
...
MakeBodyNode(const SkeletonPtr& skeleton,BodyNode* parent,Joint::Properties* joint_properties,const std::string& joint_type,dart::dynamics::Inertia inertia)
{
	...
	{
		PrismaticJoint::Properties* prop = dynamic_cast<PrismaticJoint::Properties*>(joint_properties);
		bn = skeleton->createJointAndBodyNodePair<PrismaticJoint>(
			parent,(*prop),BodyNode::AspectProperties(joint_properties->mName)).second;
	}
...
{
	std::vector<double> v = split_to_double(input, 6);
	Eigen::Vector6d res;
	res << v[0], v[1], v[2],v[3], v[4], v[5];
	return res;	
}
...
BuildFromFile(const std::string& path,bool create_obj)
{
	...
	for(TiXmlElement* node = skeleton_elem->FirstChildElement("Node");node != nullptr;node = node->NextSiblingElement("Node"))
	{
		...
		else if(type == "Prismatic")
		{
			...
			Eigen::Vector1d upper = string_to_vector1d(joint->Attribute("upper"));
			Eigen::Vector3d axis = string_to_vector3d(joint->Attribute("axis"));
			props = MASS::MakePrismaticJointProperties(name,axis,parent_to_joint,child_to_joint,lower,upper);
		}
		...
		if(obj_file!="None" && create_obj)
		{
			...
			// visual_shape->setScale(Eigen::Vector3d(0.01*1,0.01,0.01*1)*1.2);
...
// void
// MASS::
// BuildFromFile_Merge(SkeletonPtr skel, const std::string& path,bool create_obj)
// {
...
// 		std::cout << "Can't open file : " << path << std::endl;
// 	}
//     int skeleton_dof = skel->getNumDofs();
// 	std::cout<<"(DOFs : "<<skeleton_dof<<")"<< std::endl;
// 	TiXmlElement *skeleton_elem = doc.FirstChildElement("Skeleton");
// 	std::string skel_name = skeleton_elem->Attribute("name");
// 	// SkeletonPtr skel = Skeleton::create(skel_name);

	
// 	Eigen::Isometry3d humanRoot = Eigen::Isometry3d::Identity();
// 	Eigen::Isometry3d humanRoot_revised = Eigen::Isometry3d::Identity();
// 	// Eigen::Isometry3d humanInitialRelativeTransform = Eigen::Isometry3d::Identity();
// 	for(TiXmlElement* node = skeleton_elem->FirstChildElement("Node");node != nullptr;node = node->NextSiblingElement("Node"))
// 	{
...
// 		std::string parent_str = node->Attribute("parent");
// 		BodyNode* parent = skel->getBodyNode(parent_str);
// 		if(parent_str == "None")
// 			std::cout << "Something Wrong in Merge !!!!!!\n";
...
// 		T_body = Orthonormalize(T_body);	
// 		// if(name=="Pelvis"){
// 		// 	humanRoot.linear() = T_body.linear();
// 		// 	humanRoot.translation() = T_body.translation();

// 		// 	humanRoot_revised.linear() = T_body.linear() * R_y(90);
// 		// 	humanRoot_revised.translation() = T_body.translation(); 
// 		// 	humanRoot_revised.translation()(0) = humanRoot_revised.translation()(0)+0.2009;
// 		// 	humanRoot_revised.translation()(1) = humanRoot_revised.translation()(1)-0.9809;
// 		// 	humanRoot_revised.translation()(2) = humanRoot_revised.translation()(2)+0.0308;
// 		// 	humanInitialRelativeTransform = humanRoot_revised * humanRoot.inverse();
// 		// 	humanInitialRelativeTransform.setIdentity();
// 		// }
// 		// T_body =  humanInitialRelativeTransform * T_body; 
...
// 		T_joint = Orthonormalize(T_joint);	
// 		// T_joint =  humanInitialRelativeTransform * T_joint; 
...
// 	std::cout<< "Human\n" << "(DOFs : "<<skel->getNumDofs()-skeleton_dof<<")"<< std::endl;
// }
